class Tectdist < Formula
  ...
  # The installed `tectdist` dispatcher compares the actual tectonic/biber
  # pair against this declaration and fails fast with instructions when brew's
  # tectonic has moved (a mismatched pair breaks biblatex).  The weekly
  # GitHub Actions watcher and the lockstep bump procedure in RELEASING.md
  # ("Bumping the pairing") keep a matched release available before brew's
  # tectonic moves.  The formula declares TECTONIC_VERSION only as the release
  # pairing constant — no version pinning at install time.
  ...
  # biber 2.17 is BUILT FROM SOURCE here (the same approach as homebrew-core's
  # own biber formula): the plk/biber v2.17 source plus the CPAN module
  # resources below, all sha256-pinned from canonical upstreams (GitHub for
  # the biber source, metacpan for the modules) — no prebuilt binaries.

  # CPAN modules needed to build and run biber 2.17: the direct requirements
  # from plk/biber v2.17's Build.PL plus their transitive closure — exactly
  # the resource set homebrew-core's own biber formula uses (proven to build
  # in core CI), with Text::BibTeX pinned to the 2.17-era 0.89 (>= the 0.88
  # the Build.PL requires).  Installed into libexec and found via PERL5LIB.
  resource "biber" do
  ...
  # farm: every TeX tool name points at it and the dispatcher switches on the
  # invoked name.  Names provided by the dependencies above (poppler / qpdf /
  # ghostscript tools) are intentionally NOT in the farm, so the passthrough
  ...
  def install
    ...
    # Resources that must be installed earlier as they are needed to build
    # other resources (Module::Build: brew perl 5.40+ no longer bundles it;
    # Module::Install's bootstrap requires File::Remove/YAML::Tiny/Module::ScanDeps;
    # the rest: Makefile.PL-time deps of later modules).  biber itself is
    # excluded from this loop and built last, after all its requires exist.
    start_with_resources = %w[
      ...
      Capture::Tiny
      File::chdir
      File::ShareDir::Install # File::ShareDir's Makefile.PL requires it
      File::Which
      Path::Tiny
      URI
      XML::SAX::Base
    ...
    all_resources = resources.map(&:name).reject { |n| n == "biber" }
    ...
    # biber 2.17 itself (Module::Build), installed as bin/biber (plus its
    # biblex/bibparse helpers), then converted to PERL5LIB wrapper scripts.
    resource("biber").stage do
      # biber 2.17's Biber/Section.pm (del_everykeys) has a missing semicolon
      # that newer perls (5.36+) reject at compile time; upstream fixed it in
      # a later release — patch the source before building.
      inreplace "lib/Biber/Section.pm",
