...
| **X1** Supervisor + fork servers | ~90% | IPC+locks+telemetry+limits+cancellation ✓; crash/wedge recovery tested; pdfTeX AND XeTeX fork servers built & verified live (composed path, byte-identical outputs); engine-parameterized worker; format-loaded children 1.5 ms p50; LuaTeX documented (no web2c anchor, manual integration needed) |
