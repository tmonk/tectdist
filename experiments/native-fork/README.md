...
> **Status: quarantined prototype (plan O100-005 / M0).**
> Per `docs/BASICTEX_X10_PLAN.md` this work is retained as evidence and as an
> optional pdfTeX experiment only. It is not the production snapshot route:
> production acceleration goes through the `SnapshotRuntime` abstraction
> (AOT WASM primary, ArenaRuntime fallback, `NativeForkRuntime` as
> transitional pdfTeX path/benchmark control). Binaries are no longer
> committed to Git — build them in CI from the patches below.
...
- ~~`bin/forkproto/`~~ — compiled binaries were removed from source history
  (plan O100-004). Build from the patches in CI and publish as workflow
  artefacts / signed release assets.
